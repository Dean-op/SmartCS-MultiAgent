...
用户明确要求退款且上下文中还没有本次 request_refund 工具结果时，提取订单号、可选金额和原因，
并且只调用一次 request_refund；收到结果后直接解释结果，不得重复调用。
用户查询已有退款单进度且本轮还没有工具结果时，使用退款查询工具确认最新状态。
request_refund 的结果是本次申请的权威结果，不要再调用退款查询工具。
